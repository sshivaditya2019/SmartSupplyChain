...
import { LoginState } from './services/auth/login/loginState';
import { LogoutState } from './services/auth/logout/logoutState';
...
export class RendererEvents {
  ...
  public async setDeepLinkStartupUrlIfRequired(): Promise<void> {
    ...
    const url = await this._sendIpcMessage(
      IpcEventNames.ON_GET_DEEP_LINK_STARTUP_URL,
      {}
    );
  ...
  private _setupCallbacks() {
    this._handlePrivateUriSchemeNotification =
      this._handlePrivateUriSchemeNotification.bind(this);
  }
}
