...
import { OAuthConfiguration } from '../../../utils/configuration/oauthConfiguration';
import { ErrorCode as ErrorCodes } from '../../errors/errorCodes';
import { ErrorFactory } from '../../errors/errorFactory';
import { RendererEvents } from '../../../renderEvent';
...
export class LoginManager {
  ...
  private readonly _onCodeReceived: (
    code: string,
    verifier: string
  ) => Promise<void>;
  ...
  private async _startLogin(
    ...
    const browserLoginRequestHandler = new BrowserLoginRequestHandler(
      this._state,
      this._events
    );
    browserLoginRequestHandler.setAuthorizationNotifier(notifier);
    await browserLoginRequestHandler.performAuthorizationRequest(
      this._metadata,
      authorizationRequest
    );
  }
  ...
  private async _handleLoginResponse(
    ...
    if (error) {
      throw ErrorFactory.fromLoginOperation(
        error,
    ...
    try {
      ...
      // Handle any error conditions
      throw ErrorFactory.fromTokenError(
        e,
        ErrorCodes.authorizationCodeGrantFailed
      );
    }
  }
}
