...
export class BrowserLoginRequestHandler extends AuthorizationRequestHandler {
    ...
    this._authorizationPromise = new Promise<AuthorizationRequestResponse>(
      (resolve) => {
        ...
        const callback = (queryParams: any) => {
          ...
          const response = this._handleBrowserLoginResponse(
            queryParams,
            request
          );
          resolve(response);
  ...
  private _handleBrowserLoginResponse(
    ...
    const authFields = queryParams as AuthorizationResponseJson &
      AuthorizationErrorJson;
    ...
    const { state } = authFields;
    const { code } = authFields;
    const { error } = authFields;

    if (error) {
      // Handle error responses if required
      const errorDescription = authFields.error_description?.replace(
        /\+/g,
        ' '
      );
      const errorJson = {
